...
The easiest way to get PaperHeart on Windows is to download the ready-to-run executable from **[GitHub Releases](https://github.com/Nilina2002/paperHeart/releases/latest)** — no Python, no `pip install`, no command line.

1. Go to the [Releases page](https://github.com/Nilina2002/paperHeart/releases/latest).
2. Download the latest **`PaperHeart-vX.Y.Z-Windows.exe`** file.
3. Double-click it to run — that's it.

The executable is fully standalone (~38 MB, single file, includes the app icon) — copy it anywhere (a USB stick, another PC, a shared drive) and run it with no install step. To make it feel like an installed app, right-click it → *Send to* → *Desktop (create shortcut)*, or pin it to Start/Taskbar.

Every push of a version tag (e.g. `v1.0.0`) automatically builds and publishes a new release via [GitHub Actions](.github/workflows/release.yml) — see [Versioning & releasing](#versioning--releasing) below.
...
If you'd rather build it locally instead of downloading a release:
...
3. Runs PyInstaller (using `PaperHeart.spec`) to produce **`dist\PaperHeart.exe`**.
...
├── tests/
│   └── test_core.py      # Smoke tests for core.py, run in CI before every release
├── requirements.txt      # PyMuPDF + Pillow
├── build_exe.bat         # Builds dist\PaperHeart.exe in an isolated venv (see above)
├── PaperHeart.spec       # PyInstaller build spec (used locally and in CI)
├── .github/
│   └── workflows/
│       └── release.yml   # Builds & publishes a Windows .exe to GitHub Releases on a version tag
├── assets/
...
Releases are built and published automatically by [`.github/workflows/release.yml`](.github/workflows/release.yml) whenever a version tag matching `vX.Y.Z` is pushed:
...
This triggers a GitHub Actions job that runs on a Windows runner and:

1. Installs the project's dependencies.
2. Runs the automated tests in `tests/`.
3. Builds `PaperHeart.exe` with PyInstaller (`PaperHeart.spec`).
4. Publishes a GitHub Release for the tag with auto-generated release notes and the executable (named `PaperHeart-vX.Y.Z-Windows.exe`) attached.

No manual build or upload step is needed — pushing the tag is the entire release process.
